...
def index():
    ...
    tracked_files = sbac_app.status() if is_init else []
    history = sbac_app.history() if is_init else []
    baselines = sbac_app.list_baselines() if is_init else {}
    return render_template('index.html', is_init=is_init, files=tracked_files, history=history, baselines=baselines)
