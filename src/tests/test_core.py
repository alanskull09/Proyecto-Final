...
import unittest
import time
import src.sbac.core

# Aislamos las pruebas en una carpeta temporal para no tocar tu repositorio real
TEST_DIR = ".sbac_test"
src.sbac.core.SBAC_DIR = TEST_DIR
src.sbac.core.DB_FILE = os.path.join(TEST_DIR, "index.db")
src.sbac.core.OBJECTS_DIR = os.path.join(TEST_DIR, "objects")
...
class TestSBAC(unittest.TestCase):
    ...
    def test_01_init_creates_structure(self):
        ...
        res = self.app.init()
        self.assertTrue(os.path.exists(src.sbac.core.DB_FILE))
        self.assertTrue(os.path.exists(src.sbac.core.OBJECTS_DIR))
